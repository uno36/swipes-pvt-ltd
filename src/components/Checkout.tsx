  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (isSuccess) {
        ...
        onPaymentSuccess(); // Clear cart and navigate home
      } else {
        setPaymentStatus("failure");
        setMessage(
          "Payment failed. Please try again or choose another method."
        );
        onPaymentFailure(); // Do not clear cart, allow retry
      }
    } catch (error) {
      setPaymentStatus("failure");
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-white rounded-xl shadow-lg p-6">
          ...
          <button
            ...
            className="w-full bg-green-600 text-white font-semibold py-3 px-8 rounded-lg shadow-md hover:bg-green-700 transition duration-300 ease-in-out mt-8 disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          {paymentStatus === "success" && (
            <button
              onClick={handleGoToHome}
              className="w-full bg-blue-600 text-white font-semibold py-3 px-8 rounded-lg shadow-md hover:bg-blue-700 transition duration-300 ease-in-out mt-4"
            >
              Back to Shop
            </button>
          )}
